      ...
      <Component>
        {showData?.map((el, i) => (
          <Wrapper key={i}>
            <TopElement>
              {el.src  && (
                <LogoWrapper bgColor={bgColors[Math.floor(Math.random()*10/2)]}>
                  <LogoImg src={el.src && el.src} alt="J" />
                </LogoWrapper>
              )}
              {!el.src  && (
                <LogoWrapper bgColor={bgColors[Math.floor(Math.random()*10/2)]}>{el.title.slice(0, 1)}</LogoWrapper>
              )}
              {el.title && <Typography fontWeight={el.text ? 500 : 400}>{el.title}</Typography>}
